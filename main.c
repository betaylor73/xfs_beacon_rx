...
static volatile int16_t rx_bit_buffer[100] = {0};
...
void main(void)
{
    ...
    // You should now be able to see a 500 kHz square wave at pin 160 (EPWM1A)
    // with a 50% duty cycle. The SOC A event should be triggering at 1 MHz
    // (every 1 microsecond). The SOC A event is triggering at twice the 
    // frequency of the PWM because the counter is in up-down count mode and 
    // the PWM output toggles a CMPA=0 during the up-count, while the SOC A
    // event also triggers at CMPA=0 during the up-count. Since it takes two
    ...
    while (true)
    {
        ...
        // toggle health LED every 250 ms (every 5 * 50 ms)
        if (led_enabled) 
        {
            if (++led_toggle_counter >= 5)
            {
...
__interrupt void bit_rx_isr(void)
{
    ...
    if (detected_bit == 1) {
        ...
        idle_count++;
        if (idle_count > 10) { accumulating_bits = true; }
